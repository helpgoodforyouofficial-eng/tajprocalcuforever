// ramadan.js - Final Master Version
...
function initRamadanFeature() {
    ...
    // SAVED DATA CHECK: Memory se load karna
...
async function fetchLocation() {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        } catch (e) { alert("Fetch Error!"); mainBtn.innerText = "Try Again"; }
    }, (e) => { alert("Enable GPS!"); mainBtn.innerText = "GPS Error"; });
}

function startRamadanCountdown(timings) {
    if (window.ramadanTimer) clearInterval(window.ramadanTimer);
    window.ramadanTimer = setInterval(() => {
        const target = getNextTarget(timings.Fajr.split(' ')[0], timings.Maghrib.split(' ')[0]);
        const diff = target.time - new Date();
        if (diff > 0) {
...
function getNextTarget(sehri, iftari) {
    ...
    return { name: "Sehri (Tomorrow)", time: new Date(sDate.getTime() + 86400000) };
}

function handleDropChange() { /* logic remains same */ }
function saveAllAlerts() { Notification.requestPermission().then(p => { if(p === 'granted') { alert("Success! Countdown Active."); if(window.lastFetchedTimings) startRamadanCountdown(window.lastFetchedTimings); closeRamadanModal(); }}); }
